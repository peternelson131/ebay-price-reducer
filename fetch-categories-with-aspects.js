...
async function run() {
  ...
  for (let i = 0; i < leaves.length; i++) {
    ...
    try {
      ...
      // ONLY store if has required aspects
    ...
    if (processed % 500 === 0 && processed > 0) {
      ...
      console.log(`📊 ${processed}/${leaves.length} | Stored: ${stored} | Skipped: ${skipped} | ~${Math.round(remaining/60)}min left`);
    }
  ...
  console.log(`   Stored (with aspects): ${stored}`);
  console.log(`   Skipped (no aspects): ${skipped}`);
