...
async function test() {
  ...
  console.log('🔍 Checking required aspects for different categories\n');

  // Get item aspects for different categories
  const categories = [
  ...
  for (const cat of categories) {
    ...
    if (data.aspects) {
      const required = data.aspects.filter(a => a.aspectConstraint?.aspectRequired);
      console.log(`   Required aspects (${required.length}):`);
      required.forEach(a => {
        const values = a.aspectValues?.slice(0, 3).map(v => v.localizedValue).join(', ') || 'free text';
        console.log(`     - ${a.localizedAspectName}: ${values}${a.aspectValues?.length > 3 ? '...' : ''}`);
      });
    } else {
      console.log(`   Error: ${JSON.stringify(data).substring(0, 80)}`);
    }
    console.log('');
  }
}
