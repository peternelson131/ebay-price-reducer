...
async function test() {
  ...
  let resp = await fetch(`https://api.ebay.com/sell/inventory/v1/inventory_item/${encodeURIComponent(SKU)}`, {
    method: 'PUT',
    headers: { 
  ...
  if (!resp.ok) { console.log('❌ Offer error:', JSON.stringify(offer, null, 2)); return; }
  ...
  if (!resp.ok) { console.log('❌ Publish error:', JSON.stringify(pub, null, 2)); return; }
