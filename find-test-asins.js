...
// ASINs to check
const ASINS = [
  'B0DJHKTCSF',  // Video game - Luigi Mansion
  'B0BDJ279KF',  // Video game - Zelda
  'B09JSPLN1K',  // Book - Atomic Habits
  'B0931VRJT5',  // Kitchen - Instant Pot
  'B07FZ8S74R',  // Electronics - Echo Dot
  'B08N5WRWNW',  // Original test
];
...
async function run() {
  ...
  for (const asin of ASINS) {
    ...
    if (!p) { console.log(`${asin}: Not found`); continue; }
    
    const hasImages = !!(p.imagesCSV || p.images?.length);
    console.log(`${asin}: ${p.title?.substring(0, 40)}...`);
    console.log(`  Group: ${p.productGroup}, Type: ${p.type}`);
