/**
 * Toggle Logins - Admin-only endpoint to enable/disable user logins
 * 
 * GET /toggle-logins - Get current login status
 * POST /toggle-logins - Toggle login status
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (event.httpMethod === 'GET') {
      ...
      if (error) {
        // If setting doesn't exist, logins are enabled by default
      ...
      return successResponse({
        loginsDisabled: setting.value === 'true'
      }, headers);
    }
    ...
    if (event.httpMethod === 'POST') {
      ...
      console.log(`✅ Logins ${disabled ? 'disabled' : 'enabled'} by admin ${user.email}`);

      return successResponse({
        loginsDisabled: disabled,
        updated: true
      }, headers);
    }
    ...
    console.error('Toggle logins error:', error);
