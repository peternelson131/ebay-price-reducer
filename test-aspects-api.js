...
async function test() {
  ...
  console.log('🔍 Getting REQUIRED aspects for test categories\n');
  ...
  for (const cat of categories) {
    ...
    if (data.aspects) {
      ...
      const required = data.aspects.filter(a => a.aspectConstraint?.aspectRequired === true);
      
      console.log(`   Required aspects: ${required.length}`);
      required.forEach(a => {
        const mode = a.aspectConstraint?.aspectMode;
        const valueCount = a.aspectValues?.length || 0;
        console.log(`   - ${a.localizedAspectName} (${mode}, ${valueCount} values)`);
        
        // Show first 3 valid values if enumerated
        if (a.aspectValues?.length > 0) {
          const sample = a.aspectValues.slice(0, 3).map(v => v.localizedValue).join(', ');
          console.log(`     Values: ${sample}${valueCount > 3 ? '...' : ''}`);
        }
      });
    }
    console.log('');
