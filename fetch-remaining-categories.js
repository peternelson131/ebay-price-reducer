...
async function run() {
  ...
  // 1. Get already fetched categories
  console.log('📥 Getting already fetched categories...');
  ...
  console.log(`   Already have: ${existingIds.size}`);
  ...
  const allLeaves = getLeaves(tree.rootCategoryNode);
  const remaining = allLeaves.filter(l => !existingIds.has(l.id));
  console.log(`📊 Total leaves: ${allLeaves.length}, Remaining: ${remaining.length}\n`);

  // 4. Fetch aspects for remaining
  ...
  for (let i = 0; i < remaining.length; i++) {
    ...
    if (processed % 500 === 0 && processed > 0) {
      ...
      console.log(`📊 ${existingIds.size + processed}/${allLeaves.length} (${Math.round(rate)}/sec, ~${Math.round(eta/60)}min remaining)`);
    }
  ...
  const total = existingIds.size + processed;
  console.log(`\n✅ Done! Total: ${total} categories (${errors} errors this run)`);
}
