...
async function run() {
  ...
  console.log('📥 Fetching required aspects (clean format)\n');
  ...
  for (const cat of sampleCategories) {
    ...
    // Clean format - just the metadata, no values
    ...
    if (error) {
      ...
      requiredAspects.forEach((a, i) => {
        console.log(`   ${i+1}. ${a.name}`);
        console.log(`      - Data Type: ${a.dataType}`);
        console.log(`      - Cardinality: ${a.cardinality}`);
